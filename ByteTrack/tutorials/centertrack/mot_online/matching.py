...
def embedding_distance2(tracks, detections, metric='cosine'):
    ...
    track_features = np.asarray([track.smooth_feat for track in tracks], dtype=np.float_)
    cost_matrix = np.maximum(0.0, cdist(track_features, det_features, metric))  # Nomalized features
    track_features = np.asarray([track.features[0] for track in tracks], dtype=np.float_)
    cost_matrix2 = np.maximum(0.0, cdist(track_features, det_features, metric))  # Nomalized features
    track_features = np.asarray([track.features[len(track.features)-1] for track in tracks], dtype=np.float_)
...
def vis_id_feature_A_distance(tracks, detections, metric='cosine'):
    ...
    leg1 = len(tracks)
    leg2 = len(detections)
    cost_matrix = np.zeros((leg1, leg2), dtype=np.float_)
    cost_matrix_det = np.zeros((leg1, leg2), dtype=np.float_)
    cost_matrix_track = np.zeros((leg1, leg2), dtype=np.float_)
    det_features = np.asarray([track.curr_feat for track in detections], dtype=np.float_)
    track_features = np.asarray([track.smooth_feat for track in tracks], dtype=np.float_)
    if leg2 != 0:
