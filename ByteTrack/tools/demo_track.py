    ...
    def inference(self, img, timer):
        ...
        with torch.no_grad():
            ...
            if self.decoder is not None:
                outputs = self.decoder(raw_outputs, dtype=outputs.type())
            outputs = postprocess(
                raw_outputs, self.num_classes, self.confthre, self.nmsthre
            )
            #logger.info("Infer time: {:.4f}s".format(time.time() - t0))
        return raw_outputs, outputs, img_info
...
def image_demo_offline(vis_folder, current_time, args):
    ...
    dataset_name = "Charades" # "BDD"
    video_id = "5TM3H" # "b2a5baf7-58519386"
    ...
    det_path = os.path.join("/home/uig93971/src/Hyp-OW-burst-eval/detections", dataset_name, video_id + ".json")
    ...
    for frame_id, img_path in enumerate(burst_val_gt[video_key]['all_image_paths'], 1):
        # if frame_id > 536:
        #     return
        img_info = init_img_info(os.path.join(video_path, img_path))
        outputs = [preprocess_detections(detections[img_path])]
        # print('outputs')
        # print(outputs)
        if outputs[0] is not None:
            online_targets = tracker.update(outputs[0][:, :5], [img_info['height'], img_info['width']], (img_info['height'], img_info['width']))
        ...
        # result_image = predictor.visual(outputs[0], img_info, predictor.confthre)
        exp_name = "charades_08match"
        if args.save_result:
            timestamp = time.strftime("%Y_%m_%d_%H_%M_%S", current_time)
            save_folder = osp.join(vis_folder, exp_name, "frames")
            os.makedirs(save_folder, exist_ok=True)
            cv2.imwrite(osp.join(save_folder, osp.basename(img_path)), online_im)
    ...
    if args.save_result:
        res_file = osp.join(vis_folder, exp_name, f"{exp_name}.txt")
        with open(res_file, 'w') as f:
            f.writelines(results)
        logger.info(f"save results to {res_file}")
...
def imageflow_demo(predictor, vis_folder, current_time, args):
    ...
    while True:
        ...
        if ret_val:
            raw_outputs, outputs, img_info = predictor.inference(frame, timer)
            if frame_id % 20 == 0:
...
def main(exp, args):
    ...
    if args.demo == "image":
        ...
        image_demo_offline(vis_folder, current_time, args)
